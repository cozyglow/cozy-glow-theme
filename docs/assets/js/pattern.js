  ...
  setMinimumContrast(color, contrast, value) {
    ...
    if (chroma.contrast(this.color, color.color) < contrast) {
      ...
      if (this.color.luminance() === color.color.luminance()) {
        ...
        func =
          this.color.luminance() < color.color.luminance()
            ? "darken"
            : "lighten";
      }

      while (
        chroma.contrast(this.color, color.color) < contrast &&
        !this.color.luminance() !== ("darken" ? 0 : 1)
      ) {
        switch (func) {
          case "darken":
...
class Pattern {
  constructor(
    ...
    if (xColorOption !== undefined) {
      this.xColorOption = xColorOption;
    }

    if (yColorOption !== undefined) {
      this.yColorOption = yColorOption;
    }

    if (xPalette !== undefined) {
      this.xPalette = xPalette;
    }

    if (yPalette !== undefined) {
      this.yPalette = yPalette;
    }
    ...
    if (foregroundColorOption !== undefined) {
      this.foregroundColorOption = foregroundColorOption;
    }
    ...
    if (xGradientType !== undefined) {
      this.xGradientType = xGradientType;
    }

    if (yGradientType !== undefined) {
      this.yGradientType = yGradientType;
    }
  ...
  set xColorOption(xColorOption) {
    ...
    if (!xColorOptions.includes(xColorOption)) {
      console.error(
        "X color option must be either: " + xColorOptions.join(", ")
      );
      return;
    }
  ...
  set yColorOption(yColorOption) {
    ...
    if (!yColorOptions.includes(yColorOption)) {
      console.error(
        "Y color option must be either: " + yColorOptions.join(", ")
      );
      return;
    }
  ...
  set foregroundColorOption(foregroundColorOption) {
    ...
    if (!foregroundColorOptions.includes(foregroundColorOption)) {
      console.error(
        "Foreground color option must be either: " +
          foregroundColorOptions.join(", ")
      );
      return;
    }
  ...
  set xGradientType(xGradientType) {
    ...
    if (!xGradientTypes.includes(xGradientType)) {
      console.error(
        "X gradient type must be either: " + xGradientTypes.join(", ")
      );
      return;
    }
  ...
  set yGradientType(yGradientType) {
    ...
    if (!yGradientTypes.includes(yGradientType)) {
      console.error(
        "Y gradient type must be either: " + yGradientTypes.join(", ")
      );
      return;
    }
  ...
  generate() {
    ...
    if (this.yColorOption === PatternYColorOption.palette) {
      ...
      if (yGradientType === PatternYGradientType.matchX) {
